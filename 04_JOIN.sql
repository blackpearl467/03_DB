...
--> JOIN구문 잘못 작성하는 경우 CROSS JOIN의 결과가 조회됨
...
--반드시 두테이블 간의 동일한 컬럼명, 타입을 가진 컬럼이 필요
--> 없을 경우 교차조인됨
...
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN JOB USING (JOB_CODE)
--WHERE EMP_NO BETWEEN '700000-000000' AND '799999-999999'
...
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE);
--WHERE DEPT_TITLE LIKE '부';  <이거 안해도됨 ㅠㅠ
...
--SELECT EMP_NAME, JOB_NAME, SALARY, SALARY * 12 * BONUS '연봉(보너스포함)' <내가써봤던거
SELECT EMP_NAME, JOB_NAME, SALARY, SALARY * ( 1 + NVL(BONUS, 0) ) * 12 연봉
FROM EMPLOYEE
--JOIN JOB USING (JOB_CODE) <내가써봤던거
JOIN SAL_GRADE USING (SAL_LEVEL)
NATURAL JOIN JOB
WHERE SALARY > MIN_SAL; --초과
...
--JOIN JOB USING (JOB_CODE)
NATURAL JOIN JOB
WHERE BONUS IS NULL --보너스포인트가 없는
AND JOB_CODE IN ('J4', 'J7'); --JOIN 아닌 AND로