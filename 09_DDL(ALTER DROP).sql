...
-->컬럼이 생성되면서 DEFAULT값이 자동삽입되었음 을 확인

--D10 개발1팀 추가
INSERT INTO DEPT_COPY
VALUES ('D10', '개발1팀', 'L1', DEFAULT, DEFAULT);
--SQL Error [12899] [72000]: ORA-12899: "KH_KBG"."DEPT_COPY"."DEPT_ID" 열에 대한 값이 너무 큼(실제: 3, 최대값: 2)
...
ALTER TABLE DEPT_COPY DROP COLUMN CNAME; --이렇게써도됨
...
SELECT * FROM DEPT_COPY; --이제 삭제되서 조회되지않음
...
ALTER TABLE DEPT_COPY ADD CONSTRAINT D_COPY_PK PRIMARY KEY(DEPT_ID); --제약조건 PK
...
DROP TABLE TB1 CASCADE CONSTRAINTS; --테이블 삭제시 FK 관계도 모두 삭제

SELECT * FROM TB1; --삭제확인됨

SELECT * FROM TB2; --TB2 테이블만 아무관계없이 남게됨
...
ROLLBACK;
SELECT * FROM TB2; 
--롤백 안된다.. 위에서 DDL구문 중 ALTER를 사용해서 그시점에 COMMIT 됨
