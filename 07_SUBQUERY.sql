...
WHERE DEPT_CODE = (SELECT DEPT_CODE
					FROM EMPLOYEE
					WHERE EMP_NAME = '노옹철'); --괄호안에 1번 넣으면됨
				
--서브쿼리 예시 2.
--전 직원의 평균 급여보다 많은 급여를 받고있는 직원의
--사번, 이름, 직급코드, 급여 조회

-- 1) 전직원의 평균급여 조회하기 (서브쿼리)
...
WHERE SALARY > (SELECT AVG(SALARY) FROM EMPLOYEE) --전 직원의 급여 평균보다 많은(초과)
ORDER BY JOB_CODE; --직급순으로 정렬
...
FROM EMPLOYEE
JOIN JOB USING (JOB_CODE)
--LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)XXXX
...
WHERE SALARY IN (SELECT MAX(SALARY)
				FROM EMPLOYEE
				GROUP BY DEPT_CODE) --그중에 하나랑 일치하면 IN연산자!!
ORDER BY DEPT_CODE; --부서순
...
SELECT DISTINCT MANAGER_ID --DISTINCT 중복데이터 제거
FROM EMPLOYEE
...
JOIN JOB USING (JOB_CODE)
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
WHERE EMP_ID IN (SELECT DISTINCT MANAGER_ID --DISTINCT 중복데이터 제거
				FROM EMPLOYEE
				WHERE MANAGER_ID IS NOT NULL);
...
JOIN JOB USING (JOB_CODE)
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
WHERE EMP_ID NOT IN (SELECT DISTINCT MANAGER_ID --DISTINCT 중복데이터 제거
					FROM EMPLOYEE
					WHERE MANAGER_ID IS NOT NULL);
...
--3) 방법1. MIN을 이용해서 단일행 서브쿼리를 만듦
SELECT EMP_ID, EMP_NAME, JOB_NAME, SALARY
FROM EMPLOYEE
JOIN JOB USING (JOB_CODE)
WHERE JOB_NAME = '대리'
AND SALARY > (SELECT MIN(SALARY) --MIN(SALARY) = 과장직급의 최소 급여
			  FROM EMPLOYEE
			  JOIN JOB USING (JOB_CODE)
			  WHERE JOB_NAME = '과장'); --2번 괄호안에 복사
--3) 방법2. ANY를 이용하기
SELECT EMP_ID, EMP_NAME, JOB_NAME, SALARY
FROM EMPLOYEE
JOIN JOB USING (JOB_CODE)
WHERE JOB_NAME = '대리'
AND SALARY > ANY (SELECT SALARY --ANY = 과장직급의 최소 급여
				  FROM EMPLOYEE
				  JOIN JOB USING (JOB_CODE)
			  	  WHERE JOB_NAME = '과장'); --2번 괄호안에 복사
...
-- 사번, 이름, 직급, 급여 조회
--  > ALL, < ALL : 가장 큰값 보다 크냐? / 가장 작은 값보다 작냐?
--서브쿼리
SELECT SALARY
FROM EMPLOYEE
JOIN JOB USING (JOB_CODE)
...
WHERE LOCATION_ID = (SELECT LOCAL_CODE
					 FROM LOCATION
					 WHERE NATIONAL_CODE = 'KO'); --괄호안에 1번 넣기(단일행)
					 
--3)최종적으로 EMPLOYEE테이블에서 위의 결과들과 동일한 DEPT_CODE를 가진 사원을 찾기
SELECT EMP_NAME, DEPT_CODE
FROM EMPLOYEE
WHERE DEPT_CODE IN (SELECT DEPT_ID 
					...
					WHERE LOCATION_ID = (SELECT LOCAL_CODE
					 					 FROM LOCATION
										 WHERE NATIONAL_CODE = 'KO')); --괄호안에 2번 넣기
...
FROM EMPLOYEE
WHERE ENT_YN = 'Y' --퇴사한
AND SUBSTR(EMP_NO, 8, 1) = '2'; --여직원 -- (이태림)씨
...
WHERE (DEPT_CODE, JOB_CODE) = (SELECT DEPT_CODE, JOB_CODE
								FROM EMPLOYEE
								WHERE ENT_YN = 'Y'
								AND SUBSTR(EMP_NO, 8, 1) = '2'); --둘다 조회했던거 그대로 넣기 심플@@
...
WHERE (DEPT_CODE, JOB_CODE) = (SELECT DEPT_CODE, JOB_CODE
								FROM EMPLOYEE
								WHERE EMP_NAME = '노옹철') --여기까지는 노옹철도 포함
AND EMP_NAME != '노옹철'; --노옹철 제외됨
...
FROM EMPLOYEE --조인안해도됨 EMPLOYEE테이블에 다 있음
WHERE (DEPT_CODE, JOB_CODE) = (SELECT DEPT_CODE, JOB_CODE
								FROM EMPLOYEE 
								--WHERE HIRE_DATE LIKE '0%' < 이거아님
								WHERE EXTRACT(YEAR FROM HIRE_DATE) = 2000);
...
--사번, 이름, 직급코드, 급여 조회
--단, 급여와 급여 평균은 만원단위로 계산 --TRUNC(컬럼명, -4)사용 만의자리
...
WHERE SALARY > (SELECT AVG(SALARY)
				FROM EMPLOYEE SUB
				WHERE MAIN.JOB_CODE = SUB.JOB_CODE); --괄호안에 서브쿼리 복사해서 수정?

-------------------------------------------------							
--사수가 있는 직원의 사번, 이름, 부서명, 사수사번 조회
--> 상관 서브쿼리를 사용하여 각 직원의 MANAGER_ID가
--실제로 직원테이블의 EMP_ID와 일치하는지 확인							
...
JOIN JOB USING (JOB_CODE)
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
--WHERE ENT_YN = 'N'; --퇴사한 직원은 제외하는 조건이 있었음
...
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
WHERE ENT_YN = 'N'						
AND HIRE_DATE = (SELECT MIN(HIRE_DATE) --날짜가 가장 이른순
				FROM EMPLOYEE SUB
				WHERE SUB.DEPT_CODE = MAIN.DEPT_CODE)						
ORDER BY HIRE_DATE; --입사일이 빠른순으로 정렬하는 조건이 있었음							
...
--모든 직원의 이름, 직급, 급여, 전체 사원중 가장 높은 급여와의 차(급여 차)를 조회
SELECT EMP_NAME, JOB_CODE, SALARY, 
...
/*1*/FROM EMPLOYEE
/*2*/WHERE ROWNUM <= 5
/*4*/ORDER BY SALARY DESC; --DESC 내림차순
--> SELECT문의 해석 순서 때문에
--급여 상위 5명이 아니라
...
FROM (SELECT EMP_NAME, SALARY
		FROM EMPLOYEE
		ORDER BY SALARY DESC) --해석순위1위 FROM절에서 전체직원의 SALARY 내림차순 정렬
...
/*1*/FROM (SELECT DEPT_CODE, DEPT_TITLE, CEIL(AVG(SALARY)) 평균급여 --CEIL 소수점올림처리
		FROM EMPLOYEE
		LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
...
-- 사원별 급여 순위
-- RANK() OVER(정렬순서) / DENSE_RANK() OVER
...
--2000년도 이후에 들어온 사원 조회
SELECT EMP_ID, EMP_NAME, PHONE, SALARY, JOB_NAME
FROM EMPLOYEE E
--JOIN JOB ON (E.JOB_CODE = J.JOB_CODE) 실무에선 이렇게 씀 USING 지양하기...
JOIN JOB USING (JOB_CODE)
...
-- 사번, 이름, 부서코드, 직급코드, 부서명, 직급명
--단일행 다중열 서브쿼리			
SELECT EMP_ID, EMP_NAME, DEPT_CODE, JOB_CODE, DEPT_CODE, JOB_NAME
FROM EMPLOYEE
JOIN JOB USING (JOB_CODE)
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
WHERE (DEPT_CODE, JOB_CODE) = (SELECT DEPT_CODE, JOB_CODE
...
WHERE HIRE_DATE IN (SELECT MIN(HIRE_DATE)
					FROM EMPLOYEE
					WHERE ENT_YN != 'Y'
					GROUP BY DEPT_CODE)	--위에거 괄호안에 붙여넣기
ORDER BY HIRE_DATE;	
